...
"""
        ...
        try:
            ...
            print("✅ Stage 1: Content analysis complete")
    ...
    def stage2_slide_structure(self, analysis_data: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        structure_prompt = f"""
Based on this content analysis, create a detailed 15-20 slide presentation structure.

CRITICAL VISUAL CONSTRAINT: Only recommend simple, text-free visuals. When in doubt, use "TEXT ONLY".

ANALYSIS:
{analysis_data['analysis']}

REQUIREMENTS:
- Between 15-20 slides total (aim for 18 slides for optimal coverage)
- Each slide must have a specific purpose in the narrative
- Consistent information density across all slides
- Strategic placement of SIMPLE visuals only
- Strong narrative flow with logical progression
- Deep dive sections where content warrants multiple slides
- Balanced coverage across all major themes

OUTPUT FORMAT:
SLIDE 1: [Title]
Purpose: [What this slide accomplishes]
Key Message: [One sentence core message]
Content Focus: [What information to include]
Visual Strategy: [ONLY simple charts, basic diagrams, or "TEXT ONLY"]
...
[Continue for all 15-20 slides...]

VISUAL SAFETY RULES:
- Use "TEXT ONLY" when complex visuals would be needed
- Only suggest simple bar/line/pie charts with clear data
- Basic flowcharts with boxes and arrows only
- Timeline diagrams with dates/milestones
- Professional stock photos (no text overlays)
- Simple before/after comparison tables
- NEVER suggest infographics, complex diagrams, or text-heavy images

COMPREHENSIVE NARRATIVE FLOW:
- **Opening Section (Slides 1-3)**: Title, hook, context setting
- **Problem/Opportunity Section (Slides 4-6)**: Challenge definition, current state, impact analysis
- **Deep Analysis Section (Slides 7-11)**: Detailed findings, insights, data analysis, implications
- **Solutions Section (Slides 12-16)**: Recommendations, strategy, methodology, implementation
- **Impact & Action Section (Slides 17-20)**: Expected outcomes, timeline, next steps, strong conclusion
"""
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": f"You are an expert presentation architect who designs compelling 15-20 slide structures for comprehensive executive presentations. {self.brand_guidelines}"},
            ...
            print("✅ Stage 2: Slide structure complete")

            # Parse slides
            slides = []
            slide_blocks = re.split(r'SLIDE \d+:', structure_result)[1:]

            for i, block in enumerate(slide_blocks, 1):
                if i <= 20:  # Cap at 20 slides maximum
                    slides.append({
                        "number": i,
                        "structure": block.strip(),
                        "analysis": analysis_data['analysis']
                    })

            # Ensure we have at least 15 slides
            while len(slides) < 15:
                slides.append({
                    "number": len(slides) + 1,
                    "structure": f"Additional Content Slide {len(slides) + 1}",
                    "analysis": analysis_data['analysis']
                })

            print(f"📊 Generated structure for {len(slides)} slides")
            return slides
    ...
    def stage3_individual_slides(self, slides_structure: List[Dict[str, str]], content: str) -> List[str]:
        ...
        for i, slide_info in enumerate(slides_structure, 1):
            print(
                f"🔄 Generating detailed content for slide {i}/{total_slides}...")
...
"""
        ...
        print(f"✅ Stage 3: Generated {len(detailed_slides)} individual slides")
    ...
    def _polish_slides_in_chunks(self, detailed_slides: List[str], content_type: str) -> str:
        ...
        for i in range(0, len(detailed_slides), chunk_size):
            ...
            print(
                f"🔄 Polishing slides {start_slide}-{end_slide} of {total_slides}...")
...
"""
            ...
            try:
                ...
                print(f"Error polishing slides {start_slide}-{end_slide}: {e}")
                # Use original if polishing fails
                polished_chunks.append(chunk_text)

        print(f"✅ Stage 4: Brand polish complete for {total_slides} slides")
    ...
    def generate_comprehensive_script(self, content: str, content_type: str = "business") -> str:
        ...
        print("\n🔍 Stage 1: Analyzing content and creating strategic outline...")
        analysis_data = self.stage1_content_analysis(content, content_type)

        # Stage 2: Slide Structure
        print("\n🏗️ Stage 2: Designing 15-20 slide structure and narrative flow...")
        slides_structure = self.stage2_slide_structure(analysis_data)

        # Stage 3: Individual Slides
        print(
            f"\n✍️ Stage 3: Generating detailed content for {len(slides_structure)} slides...")
        detailed_slides = self.stage3_individual_slides(
            slides_structure, content)

        # Stage 4: Brand Polish
        print(
            f"\n✨ Stage 4: Applying brand polish and visual safety check across {len(detailed_slides)} slides...")
